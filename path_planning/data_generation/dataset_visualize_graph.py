...
def visualize_graph_sample(input_file: Path, graph_dir: Path, graph_file: Path, target_file: Path,density_map_file: Path,save_path: Path, show: bool = False, verbose: bool = True) -> Path:
    """Load graph_map.pkl from graph_dir"""
    if not graph_file.exists():
        raise FileNotFoundError(f"Graph file does not exist: {graph_file}")
    if not input_file.exists():
        raise FileNotFoundError(f"Input file does not exist: {input_file}")
    # sample folders use `graph_<id>_<augmentation_id>`, while map-level folders
    # use names like `prm`/`grid`; default augmentation to 0 for map folders.
    graph_name_parts = graph_dir.stem.split("_")
    if len(graph_name_parts) >= 3 and graph_name_parts[0] == "graph":
...
def process_graph_visualization(
    ...
    input_file, graph_dir,  graph_file, target_file,density_map_file,save_path, show, verbose = args
    try:
...
def collect_graph_tasks(
    ...
    if folder_key == "sample":
        get_base_path = generate_sample_base_path
        def get_graph_dirs(base_path: Path):
            return sorted(
                [d for d in base_path.iterdir() if d.is_dir() and d.name.startswith("graph_")],
                key=lambda value: value.name,
            )
    elif folder_key == "map":
        get_base_path = generate_map_path
        def get_graph_dirs(base_path: Path):
            return [base_path]
    else:
        raise ValueError(f"Invalid folder_key: {folder_key}")
    
    for case_path in selected_cases:
        sample_base_path = get_base_path(case_path)
        ...
        for road_map_type in road_map_types:
            ...
            for graph_dir in graph_dirs:
                ...
                if input_file.exists() and graph_file.exists() and (target_file.exists() or target_space == 'ignore'):
                    tasks.append((input_file, graph_dir, graph_file, target_file,density_map_file,save_path, show, verbose))
                else:
                    if not input_file.exists():

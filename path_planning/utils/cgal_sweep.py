...
class CGAL_Sweep:
    ...
    def _query_vertices_on_segment(self, u_arr, v_arr, r):
        """Find vertices within distance r of segment u->v using bbox candidates + exact CGAL distance check."""
        u_pt = self.Point_type(*u_arr.ravel().tolist())
        v_pt = self.Point_type(*v_arr.ravel().tolist())
        seg = self.Segment_type(u_pt, v_pt)
        # Conservative bounding-box query: all vertices within r of the segment's bbox
        query_min = np.minimum(u_arr, v_arr).ravel() - r
        query_max = np.maximum(u_arr, v_arr).ravel() + r
        center = (query_min + query_max) / 2
        half_diag = float(np.linalg.norm(query_max - query_min) / 2)
        candidates = self.vertex_kdtree.query_ball_point(center.reshape(1, -1), half_diag)[0]
        # Filter with exact CGAL segment-to-point distance
        result = [idx for idx in candidates if squared_distance(seg, self.vertices[idx]) ** 0.5 < r]
        return [result]
    ...
    def overlapping_interval_cgal(
        ...
        if u == v:
            ...
            if get_time_interval:
                candidate_edge_indices = np.array(
                    [edge_idx for edge_idx in candidate_edge_indices if squared_distance(traversal_seg, self.edges[edge_idx])**0.5 < r],
                    dtype=int,
                )
                ...
                overlapping_edges = {(src_indices[ii], tgt_indices[ii]): (0, float('inf')) for ii in range(len(candidate_edge_indices))}
            else:
        ...
        if candidate_edge_indices:
            if get_time_interval:
                ...
                for t_val, is_start in [(t1_c, True), (t2_c, False)]:
                    p_tau = u_arr_f + t_val[:, np.newaxis] * vel_vec
                ...
                overlapping_edges = {
                    (src_indices[ii], tgt_indices[ii]): (float(tau_start[ii]), float(tau_end[ii]))
                    for ii in range(len(candidate_edge_indices))
                    if not no_collision[ii]
                }
            else:
                candidate_edge_indices = np.array(candidate_edge_indices, dtype=int)
                src_indices = np.array([self.edge_indices[i][0] for i in candidate_edge_indices], dtype=int)

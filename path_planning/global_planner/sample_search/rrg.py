...
class RRG(BasePathPlanner):
    ...
    def plan(self) -> Union[List[Tuple[float, ...]], Dict[str, Any]]:
        ...
        for _ in range(self.sample_num):
            ...
            node_near = self._get_nearest_node(nodes, node_rand, faiss_index, faiss_nodes)
    ...
    def _get_nearest_node(self, nodes: Dict[Tuple[float, ...], Node], 
                         node_rand: Node, index=None, faiss_nodes=None) -> Node:
        """
    ...
    def _get_nearby_nodes(self, nodes: Dict[Tuple[float, ...], Node],
                         node_list: List[Tuple[float, ...]],
                         node_new: Node, node_new_idx: int,
                         index=None, faiss_nodes=None) -> List[Tuple[Tuple[float, ...], int]]:
        """
        ...
        if self.use_faiss and index is not None and faiss_nodes is not None:
            ...
            distances, indices = index.search(query, min(index.ntotal, len(faiss_nodes)))

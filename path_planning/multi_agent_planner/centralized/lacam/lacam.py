...
class LaCAM:
    ...
    def solve(
        ...
        self.scatter_cost_margin: int = scatter_cost_margin
        self.seed: int = seed
    ...
    def _solve(self) -> Configs:
        ...
        # set distance tables
        self.dist_tables = [DistTable(self.graph_map, g) for g in self.goals]

        # optionally construct scatter hints
        scatter_obj = None
        if self.use_scatter:
            from path_planning.multi_agent_planner.centralized.lacam.scatter import (
                Scatter,
            )
            ...
            scatter_obj = Scatter(
                ...
                cost_margin=self.scatter_cost_margin,
            )
            scatter_obj.construct()
            self.info(
                1,
                f"scatter constructed with {scatter_obj.collision_table.collision_cnt} collisions",
            )

        # set PIBT (low‑level planner)
        self.pibt = PIBT(
            dist_tables=self.dist_tables,
            goals=self.goals,
            seed=self.seed,
            flg_swap=self.flg_swap,
            scatter=scatter_obj,
        )

...
from path_planning.multi_agent_planner.centralized.lacam.utility import Coord,is_valid_coord,get_neighbors
...
class DistTable:
    ...
    def __post_init__(self) -> None:
        ...
        self.NIL = self.graph_map.size
        self.Q = deque([self.goal])
        self.table = np.full(self.graph_map.shape, self.NIL, dtype=int)
    ...
    def get(self, target: Coord) -> int:
        ...
        """
        # check valid input
        if not is_valid_coord(self.graph_map, target):
            return self.graph_map.size
        ...
        while len(self.Q) > 0:
            u = self.Q.popleft()
            d = int(self.table[u])
            for v in get_neighbors(self.graph_map, u):
                if d + 1 < self.table[v]:
                    self.table[v] = d + 1
                    self.Q.append(v)
            if u == target:

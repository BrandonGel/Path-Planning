"""
@file: env.py
@breif: 2-dimension environment
@author: Winter
@update: 2023.1.13
"""
from math import sqrt
from abc import ABC, abstractmethod
from scipy.spatial import cKDTree
import numpy as np

from .node import Node

class Env(ABC):
    """
    Class for building 2-d workspace of robots.

    Parameters:
        x_range (int): x-axis range of enviroment
        y_range (int): y-axis range of environmet

    Examples:
        >>> from python_motion_planning.utils import Env
        >>> env = Env(30, 40)
    """
    def __init__(self, x_range: int, y_range: int) -> None:
        # size of environment
        self.x_range = x_range  
        self.y_range = y_range

    @property
    def grid_map(self) -> set:
        return {(i, j) for i in range(self.x_range) for j in range(self.y_range)}

    @abstractmethod
    def init(self) -> None:
        pass

class Grid(Env):
    """
    Class for discrete 2-d grid map.
    """
    def __init__(self, x_range: int, y_range: int, rr: float, obstacle_list = None) -> None:
        super().__init__(x_range, y_range)
        # allowed motions
        self.motions = [Node((-1, 0), None, 1, None), Node((-1, 1),  None, sqrt(2), None),
                        Node((0, 1),  None, 1, None), Node((1, 1),   None, sqrt(2), None),
                        Node((1, 0),  None, 1, None), Node((1, -1),  None, sqrt(2), None),
                        Node((0, -1), None, 1, None), Node((-1, -1), None, sqrt(2), None)]
        # obstacles
        self.obstacles = None
        self.obstacle_list = obstacle_list
        self.rr = rr
        self.obstacles_tree = None
        self.init()
    
    def init(self) -> None:
        """
        Initialize grid map.
        """
        x, y = self.x_range, self.y_range
        obstacles = set()
        all_obs = set()
        
        expanded_obs = set()

        if self.obstacle_list is None:
        # boundary of environment
            for i in range(x):
                obstacles.add((i, 0))
                obstacles.add((i, y - 1))
            for i in range(y):
                obstacles.add((0, i))
                obstacles.add((x - 1, i))

            # MAP1       
            # for i in range(10, 21):
            #     obstacles.add((i, 15))
            # for i in range(15):
            #     obstacles.add((20, i))
            # for i in range(15, 30):
            #     obstacles.add((30, i))
            # for i in range(16):
            #     obstacles.add((40, i))

            # MAP2
            # for i in range(25):
            #     obstacles.add((9, i))
            #     obstacles.add((21, i))
            #     obstacles.add((33, i))
            #     obstacles.add((44, i))

            # for i in range(5,30):
            #     obstacles.add((15, i))
            #     obstacles.add((27, i))
            #     obstacles.add((39, i))

            # MAP3
            # for i in range(10, 21):
            #     obstacles.add((i, 15))
            # for i in range(15):
            #     obstacles.add((20, i))
            # for i in range(12, 27):
            #     obstacles.add((30, i))
            # for i in range(15,30):
            #     obstacles.add((i, 26))
            # for i in range(23, 30):
            #     obstacles.add((i, 12))
            # for i in range(16):
            #     obstacles.add((40, i)) 

        else:
            for i in self.obstacle_list:
                obstacles.add(i)


        for i in obstacles:
            all_obs.add(i)

        if int(self.rr) == self.rr:
            rr = int(self.rr)
        else:
            rr = int(self.rr) + 1

        self.user_obs = obstacles
        #post processing
        for i in range(rr):
            for i in obstacles:
                l = [(i[0] + 1, i[1]), (i[0], i[1] + 1), (i[0] - 1, i[1]), (i[0], i[1] - 1)]
                for j in l:
                    if j not in obstacles:
                        all_obs.add(j)
                        expanded_obs.add(j)

            obstacles = all_obs.copy()
        
        self.obstacles = all_obs
        self.expanded_obs = expanded_obs
        self.obstacles_tree = cKDTree(np.array(list(obstacles)))

    def update(self, obstacles):
        self.obstacles = obstacles 
        self.obstacles_tree = cKDTree(np.array(list(obstacles)))


class Map(Env):
    """
    Class for continuous 2-d map.
    """
    def __init__(self, x_range: int, y_range: int, rr: float, obs=None) -> None:
        super().__init__(x_range, y_range)
        self.rr = rr
        self.boundary = []
        self.obs_circ = []
        self.obs_rect = []
        self.original_rects = None
        self.obs = obs
        self.original_boundaries = None
        self.init()

    def init(self):
        """
        Initialize map.
        """
        x, y = self.x_range, self.y_range
        if not self.obs:
            print("HERHE")
            # boundary of environment
            self.boundary = [
                [0, 0, 1, y],
                [0, y, x, 1],
                [1, 0, x, 1],
                [x, 1, 1, y]
            ]

            # user-defined obstacles

            # self.obs_rect = [
            #     [9, 0, 1, 24],
            #     [21, 0, 1, 24],
            #     [33, 0, 1, 24],
            #     [44, 0, 1, 24],
            #     [15, 6, 1, 25],
            #     [27, 6, 1, 25],
            #     [39, 6, 1, 25]
            # ]

            # self.obs_rect = [
            #     [10, 15, 11, 1],
            #     [20, 0, 1, 15],
            #     [30, 12, 1, 15],
            #     [15, 26, 15, 1],
            #     [23, 12, 7, 1],
            #     [40, 0, 1, 16]
            # ]

            self.obs_rect = [
                [10, 15, 11, 1],
                [20, 0, 1, 15],
                [30, 15, 1, 16],
                [40, 0, 1, 16]
            ]

            self.obs_circ = [
                
            ]
        
        else:
            self.obs_rect = self.obs

        #postprocessing
        rects = []
        boundaries = []
        for (ox, oy, h, w) in self.obs_rect:
            rects.append([ox-self.rr, oy-self.rr, h+2*self.rr, w+2*self.rr])

        for (ox, oy, h, w) in self.boundary:
            boundaries.append([ox-self.rr, oy-self.rr, h+2*self.rr, w+2*self.rr])
        
        self.original_rects = self.obs_rect
        self.original_boundaries = self.boundary
        self.obs_rect = rects
        self.boundary = boundaries
        
    def update(self, boundary, obs_circ, obs_rect):
        self.boundary = boundary if boundary else self.boundary
        self.obs_circ = obs_circ if obs_circ else self.obs_circ
        self.obs_rect = obs_rect if obs_rect else self.obs_rect

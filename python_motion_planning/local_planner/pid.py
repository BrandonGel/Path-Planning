...
from python_motion_planning.utils import Env, Grid, Map
...
class PID(LocalPlanner):
    ...
    Examples:
        ...
        >>> planner.run()
    """
    def __init__(self, start: tuple, goal: tuple, env: Env, algorithm: str, heuristic_type: str = "euclidean", **params) -> None:
        ...
        if isinstance(env, Grid) or algorithm == "prm":
            self.g_planner = {"planner_name": algorithm, "start": g_start, "goal": g_goal, "env": env}
        elif algorithm == "rrt":
            self.g_planner = {"planner_name": algorithm, "start": g_start, "goal": g_goal, "env": env, 'max_dist': 1.5,  'sample_num': 30000}
        else:
            self.g_planner = {"planner_name": algorithm, "start": g_start, "goal": g_goal, "env": env, 'max_dist': 1.5,  'sample_num': 30000, 'r': 10}
        # self.path = self.g_path[::-1]
        self.tottime = 0
        self.vel_data = []

    def __str__(self) -> str:
        return "PID Planner; Algorithm " + str(self.g_planner_)
    ...
    def plan(self):
        ...
        for ii in range(self.params["MAX_ITERATION"]):
    ...
    def linearRegularization(self, v_d: float) -> float:
        ...
        k_v_i = 0.00
        k_v_d = 0.05
        v_inc = min(k_v_p * e_v + k_v_i * self.i_v + k_v_d * d_v, self.params["MAX_V_INC"])

        v = self.robot.v + v_inc
        return min(v, self.params["MAX_V"])
    ...
    def angularRegularization(self, w_d: float) -> float:
        ...
        k_w_i = 0.00
        k_w_d = 0.01
        w_inc = min(k_w_p * e_w + k_w_i * self.i_w + k_w_d * d_w, self.params["MAX_W_INC"])

        w = self.robot.w + w_inc

        return min(w, self.params["MAX_W"])
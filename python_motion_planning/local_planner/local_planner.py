...
class LocalPlanner(Planner):
    ...
    def __init__(self, start: tuple, goal: tuple, env: Env, heuristic_type: str="euclidean", **params) -> None:
        ...
        self.params["MAX_ITERATION"] = params["MAX_ITERATION"] if "MAX_ITERATION" in params.keys() else 3000
        self.params["LOOKAHEAD_TIME"] = params["LOOKAHEAD_TIME"] if "LOOKAHEAD_TIME" in params.keys() else 1.0
        self.params["MIN_LOOKAHEAD_DIST"] = params["MIN_LOOKAHEAD_DIST"] if "MIN_LOOKAHEAD_DIST" in params.keys() else 2.5
        ...
        # self.params["MAX_V"] = params["MAX_V"] if "MAX_V" in params.keys() else 2.0
        # self.params["MIN_V"] = params["MIN_V"] if "MIN_V" in params.keys() else 0.0
        self.params["MAX_V_INC"] = 0.01
        self.params["MAX_V"] = 2.0
        self.params["MIN_V"] = 0.0
        self.params["MAX_W_INC"] = params["MAX_W_INC"] if "MAX_W_INC" in params.keys() else math.pi 
        self.params["MAX_W"] = params["MAX_W"] if "MAX_W" in params.keys() else math.pi 
        self.params["MIN_W"] = params["MIN_W"] if "MIN_W" in params.keys() else 0.0
        self.params["GOAL_DIST_TOL"] = params["GOAL_DIST_TOL"] if "GOAL_DIST_TOL" in params.keys() else 0.8
    ...
    def getLookaheadPoint(self):
        ...
        idx_prev = idx_goal - 1
        if idx_closest == len(self.path) - 1 and self.can_be_goal == len(self.path) - 2:
            idx_goal = len(self.path) - 1
        else:
            for i in range(self.can_be_goal, len(self.path)):
                if self.dist(self.path[i], (self.robot.px, self.robot.py)) >= self.lookahead_dist:
                    if i >= self.can_be_goal:
                        self.can_be_goal = i
                        idx_goal = i
                        break
        ...
        try:
            ...
            print("CANNOT SOLVE FOR THE PATH")
            return -1
        ...
        cosB = max((a * a + c * c - b * b) / (2 * a * c+1e-6), -1.00)

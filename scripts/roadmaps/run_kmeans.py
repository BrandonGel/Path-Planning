"""
Run CTopPRM with graph-aware k-means clustering on a 2D map with agents.
python scripts/roadmaps/run_kmeans.py
"""
...
if __name__ == "__main__":
    ...
    # k-means seeding: agent endpoints stay fixed centers, plus free
    ...
    print(f"CTopPRM (kmeans): {time.time() - st:.2f}s, seeds "
    ...
    vis.ax.scatter(fixed_pts[:, 0], fixed_pts[:, 1], c='black', marker='*',
                   s=200, zorder=60, label='Fixed centers (endpoints)')
    if len(free_pts):
        vis.ax.scatter(free_pts[:, 0], free_pts[:, 1], c='black', marker='^',
                       s=120, zorder=60, label='k-means / refinement seeds')
    vis.ax.legend(loc='upper right', fontsize=8)
    vis.savefig(f'{OUT_DIR}/kmeans_2d.png')
    vis.close()
    ...
    vis.savefig(f'{OUT_DIR}/kmeans_roadmap_2d.png')
    vis.close()

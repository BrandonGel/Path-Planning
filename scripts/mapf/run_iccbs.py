...
from path_planning.multi_agent_planner.centralized.ccbs.graph_generation import Environment
...
if __name__ == "__main__":
    os.makedirs("figs/iccbs", exist_ok=True)
    os.makedirs("path_planning/maps/2d/iccbs", exist_ok=True)

    set_global_seed(42)
    num_perm = 5
    for agent_radius in [1.0, 2.0]:
        ...
        #Searching
        st = time.time()
        env = Environment(map_,{},agents,10000,agent_radius,0)
        st = time.time()
        iccbs = ICCBS(env)
        solution = iccbs.search()
        ft = time.time()
        ...
        write_to_yaml(output, f"path_planning/maps/2d/iccbs/solution_radius{agent_radius}.yaml")
        ...
        #     vis.plot_path(path)
        # vis.savefig(f"figs/ccbs/ccbs_2d_radius{agent_radius}.png")
        # vis.show()
        # vis.close()

        # # Create animation
        # schedule = {"schedule": deepcopy(solution)}
        # gif_filename = f"figs/ccbs/ccbs_2d_radius{agent_radius}.gif"
        # vis = Visualizer2D()
        # vis.animate(
        #     gif_filename,
        #     map_,
    ...
    for agent_velocity in [0.0, 1.0, 2.0]:
        ...
        #Searching
        st = time.time()
        env = Environment(map_,{},agents,10000,agent_radius,agent_velocity)
        st = time.time()
        iccbs = ICCBS(env)
        solution = iccbs.search()
        ft = time.time()
        ...
        output["cost"] = env.compute_solution_cost(solution)
        output["runtime"] = ft - st
        write_to_yaml(output, f"path_planning/maps/2d/iccbs/solution_velocity{agent_velocity}.yaml")
        ...
        # vis.savefig(f"figs/iccbs/iccbs_2d_velocity{agent_velocity}.png")
        # vis.show()
        # vis.close()

        # # Create animation
        # schedule = {"schedule": deepcopy(solution)}
        # gif_filename = f"figs/iccbs/iccbs_2d_velocity{agent_velocity}.gif"
        # vis = Visualizer2D()
        # vis.animate(
        #     gif_filename,
        #     map_,

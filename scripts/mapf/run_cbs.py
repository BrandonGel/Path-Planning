...
if __name__ == "__main__":
    ...
    for agent_radius in [0.0, 1.0, 2.0]:
        ...
        write_to_yaml(output, f"path_planning/maps/2d/cbs/solution_radius{agent_radius}.yaml")
        ...
        for agent_name, trajectory in solution.items():
            ...
            vis.plot_path(path)
        vis.savefig(f"figs/cbs/cbs_2d_radius{agent_radius}.png")
        vis.show()
        vis.close()

        # Create animation
        schedule = {"schedule": deepcopy(solution)}
        gif_filename = f"figs/cbs/cbs_2d_radius{agent_radius}.gif"
        vis = Visualizer2D()
        vis.animate(
            gif_filename,
            map_,

...
if __name__ == "__main__":
    ...
    for agent_radius in [1.0, 2.0]:
        ...
        write_to_yaml(output, f"path_planning/maps/2d/ccbs/solution_radius{agent_radius}.yaml")
    ...
    for agent_velocity in [0.0, 1.0, 2.0]:
        ...
        output["cost"] = env.compute_solution_cost(solution)
        output["runtime"] = ft - st
        write_to_yaml(output, f"path_planning/maps/2d/ccbs/solution_velocity{agent_velocity}.yaml")
        ...
        vis.savefig(f"figs/ccbs/ccbs_2d_velocity{agent_velocity}.png")
        vis.show()
        vis.close()

        # Create animation
        schedule = {"schedule": deepcopy(solution)}
        gif_filename = f"figs/ccbs/ccbs_2d_velocity{agent_velocity}.gif"
        vis = Visualizer2D()
        vis.animate(
            gif_filename,
            map_,

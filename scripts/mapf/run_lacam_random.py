...
python scripts/run_lacam_random.py
"""
...
if __name__ == "__main__":
    ...
    write_to_yaml(output, f"path_planning/maps/2d/lacam_random/solution.yaml")
    ...
    for agent_name, trajectory in solution.items():
        ...
        vis.plot_path(path)
    vis.savefig(f"figs/lacam_random/lacam_random_2d.png")
    vis.show()
    vis.close()

    # Create animation
    schedule = {"schedule": deepcopy(solution)}
    gif_filename = f"figs/lacam_random/lacam_random_2d.gif"
    vis = Visualizer2D()
    vis.animate(
        gif_filename,
        map_,

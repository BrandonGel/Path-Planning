...
if __name__ == "__main__":
    ...
    parser.add_argument("-rmt","--road_map_types", type=str, nargs='+', default=['grid'], choices=['grid', 'prm', 'planar'], help="roadmap types")
    ...
    for road_map_type in road_map_types:
        print(f"Labeling dataset for road map type: {road_map_type}")
        for idx, path in enumerate(paths):
            label_dataset(path, graph_file_name, mapf_solver_name, road_map_type, agent_velocity, visualize, num_workers=num_workers)
            print(f"Path Trajectory -- [{idx+1}/{len(paths)}] Complete for road map type: {road_map_type}!")
